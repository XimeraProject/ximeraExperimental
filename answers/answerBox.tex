...
\end{explanation}
    ...
    The default is to render the answer as \handoutAnswerFormat \ when using the \verb|handout| option and otherwise render as {\defaultAnswerFormat $x^2 + 3x + 1$}\footnote{Some formatting is ignored in the online version. At the time of writing, the default format is blue.}.
...
\end{example}
    ...
    % \section{How the answer box validates an answer}
    ...
    \section{Optional Arguments}
        ...
        See \nameref{ch:AnswerPDF} for more details about adjusting the how answers are rendered in the \verb|pdf|.
...
\end{problem}
        ...
        Most of these options are ``key-value'' pairs, meaning that they have the form ``key=value''.% I outline the key names below and will give examples of values one might use for the given key.
        ...
        \begin{description}
            ...
            See \nameref{ch:validators} for options. 
            % As a result, it is \textit{far} superior to the validator environment, but it also only works from a single answer box, which makes it difficult to use for more complex problems. 
